...
| 插件 ID | 名称 | 版本 | 说明 |
| --- | --- | --- | --- |
| `AudiencesSignIn` | 观众站点签到 | 1.5.1 | 自动完成观众（Audiences）等 NexusPHP 站点的每日签到；HTTP 探测 + 浏览器过人机验证两段式；支持设置页一键「立即运行一次」；浏览器页面证据日志便于排障；导航前注入 Cookie 罐并对齐 UA 以保障登录态；验证词判定加登录态反证，避免把已签到的成功页误判为需验证；HTML 转文本先删注释再剥标签，避免注释碎片污染判定 |
